...
/* cl kernel */
const char *kernel_source = "opencl/devGolGenerateShr.cl";
...
int main(int argc, char *argv[])
{
    ...
    printf("global_work_size[0]=%lu, local_work_size[0]=%lu, elements_size[0]=%lu, work_groups_x=%lu\n",
...
void generationTimer(int value)
{
    ...
    // add them 3 dong ben duoi
    //glBindBuffer(GL_PIXEL_UNPACK_BUFFER, rendered_texture);
    //glBindTexture(GL_TEXTURE_2D, rendered_texture);
    //glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, gol_map_width, gol_map_height, GL_RGBA, GL_UNSIGNED_BYTE, NULL);
...
void display()
{
    ...
    // glTexImage2D :  declares a texture’s size, color components (RGBA, etc), datatype (byte, float...), pixel data,
